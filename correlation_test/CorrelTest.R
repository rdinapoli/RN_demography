...
spd.nnorm.palms.cor <- cor.test(nnrm$y, nnrm$palms)
spd.nnorm.palms.cor
spd.nnorm.soi.cor <- cor.test(nnrm$y, nnrm$SOI)
spd.nnorm.soi.cor
spd.norm.palms.cor <- cor.test(nrm$y, nrm$palms)
spd.norm.palms.cor
spd.norm.soi.cor <- cor.test(nrm$y, nrm$SOI)
spd.norm.soi.cor
...
spd.norm.palms.cor.pre <- cor.test(nrm_pre$x, nrm_pre$palms)
spd.norm.palms.cor.pre
spd.norm.palms.cor.post <- cor.test(nrm_post$y, nrm_post$palms)
spd.norm.palms.cor.post
spd.norm.soi.cor.pre <- cor.test(nrm_pre$y, nrm_pre$SOI)
spd.norm.soi.cor.pre
spd.norm.soi.cor.post <- cor.test(nrm_post$y, nrm_post$SOI)
spd.norm.soi.cor.post
spd.nnorm.palms.cor.pre <- cor.test(nnrm_pre$y, nnrm_pre$palms)
spd.nnorm.palms.cor.pre
spd.nnorm.palms.cor.post <- cor.test(nnrm_post$y, nnrm_post$palms)
spd.nnorm.palms.cor.post
spd.nnorm.soi.cor.pre <- cor.test(nnrm_pre$y, nnrm_pre$SOI)
spd.nnorm.soi.cor.pre
spd.nnorm.soi.cor.post <- cor.test(nnrm_post$y, nnrm_post$SOI)
spd.nnorm.soi.cor.post

# Create data.frames for plotting
norm.palms.pre <- data.frame(name='Pre-490 BP',SPD='normalized',r=spd.norm.palms.cor.pre$estimate, 
                             low=spd.norm.palms.cor.pre$conf.int[1], high=spd.norm.palms.cor.pre$conf.int[2])
norm.palms.post <- data.frame(name='Post-490 BP',SPD='normalized',r=spd.norm.palms.cor.post$estimate, 
                              low=spd.norm.palms.cor.post$conf.int[1], high=spd.norm.palms.cor.post$conf.int[2])
norm.soi.pre <- data.frame(name='Pre-490 BP',SPD='normalized',r=spd.norm.soi.cor.pre$estimate, 
                           low=spd.norm.soi.cor.pre$conf.int[1], high=spd.norm.soi.cor.pre$conf.int[2])
norm.soi.post <- data.frame(name='Post-490 BP',SPD='normalized',r=spd.norm.soi.cor.post$estimate, 
                            low=spd.norm.soi.cor.post$conf.int[1], high=spd.norm.soi.cor.post$conf.int[2])
nnorm.palms.pre <- data.frame(name='Pre-490 BP',SPD='non-normalized',r=spd.nnorm.palms.cor.pre$estimate, 
                              low=spd.nnorm.palms.cor.pre$conf.int[1], high=spd.nnorm.palms.cor.pre$conf.int[2])
nnorm.palms.post <- data.frame(name='Post-490 BP',SPD='non-normalized',r=spd.nnorm.palms.cor.post$estimate, 
                               low=spd.nnorm.palms.cor.post$conf.int[1], high=spd.nnorm.palms.cor.post$conf.int[2])
nnorm.soi.pre <- data.frame(name='Pre-490 BP',SPD='non-normalized',r=spd.nnorm.soi.cor.pre$estimate, 
                            low=spd.nnorm.soi.cor.pre$conf.int[1], high=spd.nnorm.soi.cor.pre$conf.int[2])
nnorm.soi.post <- data.frame(name='Post-490 BP',SPD='non-normalized',r=spd.nnorm.soi.cor.post$estimate, 
                             low=spd.nnorm.soi.cor.post$conf.int[1], high=spd.nnorm.soi.cor.post$conf.int[2])
...
p4 <- ggplot(data=nrm, aes(x=x, y=y)) + 
  ...
  xlim(800,200) +
  geom_vline(xintercept=490, linetype='dashed', color='black', size=1)
cor.plot1 <- ggplot(data=cor.results.prepost.palms, aes(x=r, y=name, color=SPD)) + geom_point() +
  geom_errorbar(aes(xmin=low, xmax=high), width=0.1) +
  xlim(-1,1) +
  geom_vline(xintercept = 0, linetype='dashed') +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  labs(x = "r ", y="Palm cover") +
  labs(subtitle="e) correlation between palm cover and SPDs")
cor.plot2 <- ggplot(data=cor.results.prepost.soi, aes(x=r, y=name, color=SPD)) + geom_point() +
  geom_errorbar(aes(xmin=low, xmax=high), width=0.1) +
  xlim(-1,1) +
  geom_vline(xintercept = 0, linetype='dashed') +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  labs(x = "r ", y="SOI index") +
