...
# Model 3 ####
postpcheck.model3= vector('list',length=8)
postpcheck.model3[1:8] = rep(list(matrix(NA,nrow=abs(diff(timeRange))+1,ncol=nrep)),8)
...
for (k in 1:8)
{
  ...
  for (i in 1:nrep)
  {
    postpcheck.model3[[k]][,i] = GrowthModel(nt0=post$nt0[i],r=post$r[i],timeRange=timeRange,b1=post$b1[i],b2=post$b2[i],x1=x1,x2=x2,a=1,raw = TRUE)$N
  }
}
...
for(i in 1:4)
{
  plot(timeRange[1]:timeRange[2],xlim=timeRange,ylim=range(postpcheck.model3[[i]]),xlab='Cal BP',ylab='N',main=titles[i])
  apply(postpcheck.model3[[i]],2,lines,x=timeRange[1]:timeRange[2],col=rgb(0,0,0,0.05))
}
dev.off()
...
for(i in 5:8)
{
  plot(timeRange[1]:timeRange[2],xlim=timeRange,ylim=range(postpcheck.model3[[i]]),xlab='Cal BP',ylab='N',main=titles[i-4])
  apply(postpcheck.model3[[i]],2,lines,x=timeRange[1]:timeRange[2],col=rgb(0,0,0,0.05))
}
dev.off()
